...
while [[ $# -gt 0 ]]; do
	case "$1" in
		-h | --help)
			...
			echo "    "
			echo "    c♭ generates code for x86-64 (aka x64/AMD64/Intel 64) using the SYS V ABI."
			echo "    c♭ requires recent versions of: Bash, Perl, binutils (as and ld) or gcc"
			echo "    (with --use-gcc), coreutils (mktemp, sort, head, base64, and tr), find, and"
			echo "    util-linux-ng's getopt."
			echo "    Unless --use-gcc is passed, GCC C runtime libraries are required. By"
			echo "    default, the standard GCC installation directories are searched, but this"
			echo "    can be overridden with --libroot and --gccroot (see below). libroot must"
			echo "    contain Scrt1.o, crti.o, and crtn.o, and gccroot must contain crtbeginS.o"
			echo "    and crtendS.o. libroot, gccroot, /lib, /usr/lib, or /lib/x86_64-linux-gnu"
			echo "    must contain libgcc, libgcc_s, and libc and their dependencies. The compiled"
			echo "    binaries use /lib64/ld-linux-x86-64.so.2 as the dynamic linker."
			echo "    However, if --use-gcc is passed, GCC's defaults are used instead."
			echo "    "
			echo "    Options:"
